...
export default function StatsSidebarHeader({
  ...
  if(isMobile){
    return (
      <div
        className="flex items-center justify-between  px-1  font-semibold text-black">
        <div className="min-w-0" style={{ flex: 1 }}>
          <h2 className="text-[18px] text-black truncate dark:text-white">{title}</h2>
        </div>
        <div></div>
      </div>
  ...
  return (
    <div
      className="flex items-center justify-between  border-b-slate-800/20 px-4  font-semibold text-black ">
      <div className="min-w-0" style={{ flex: 1 }}>
        <h2 className="text-[24px] text-black truncate dark:text-white">{title}</h2>
      </div>
